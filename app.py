...
def fetch_live_matches_and_odds():
  ...
  # 1. Recupero da RapidAPI (Tennis API ATP WTA ITF)
  if rapidapi_key:
    try:
  ...
  # 2. Recupero alternativo da The-Odds-API (se la prima API non ha restituito match)
      ...
      if odds_res.status_code == 200:
        ...
        for item in odds_data:
          ...
          if item.get("bookmakers"):
            ...
            h2h = next(
                (m for m in bm["markets"] if m["key"] == "h2h"), None
            )
            if h2h:
              p1_price = next(
                  (
              ...
              matches[title] = {
                  "p1": item["home_team"],
                  "p2": item["away_team"],
                  "tour": item.get("sport_title", "ATP/WTA"),
                  "surf": "Cemento/Terra",
                  "elo1": 1680,
                  "elo2": 1600,
                  "g1": 12.5,
                  ...
                  "df1": 2.0,
                  "df2": 2.8,
                  "o_win1": float(p1_price),
                  "o_win2": float(p2_price),
                  "o_u215": 1.83,
                  "o_set20": 2.45,
              }
    except Exception:
  ...
  # Fallback Database Demo (se nessuna API risponde o le chiavi non sono valide)
  if not matches:
    matches = {
