...
# ==============================================================================
# INTEGRATORE API AUTOMATICO (LETTURA DA SECRETS O BACKUP)
# ==============================================================================
@st.cache_data(ttl=1800)
def fetch_live_matches_and_odds():
    matches = {}
    
    # Lettura automatica delle API Keys dai Secrets di Streamlit se configurati
    ...
    if rapidapi_key:
        try:
            url = "https://tennis-api-provider.p.rapidapi.com/v1/matches/today"
            headers = {"X-RapidAPI-Key": rapidapi_key}
            res = requests.get(url, headers=headers, timeout=5)
            if res.status_code == 200:
                data = res.json()
                for ev in data.get("events", []):
                    title = f"{ev['home_player']['name']} vs {ev['away_player']['name']} ({ev['tournament']['name']})"
                    matches[title] = {
                        "p1": ev["home_player"]["name"],
                        "p2": ev["away_player"]["name"],
                        "tour": ev["tournament"]["name"],
                        "surf": ev["tournament"].get("surface", "Hard"),
                        "elo1": ev["home_player"].get("surface_elo", 1650),
                        "elo2": ev["away_player"].get("surface_elo", 1580),
                        "g1": 12.2, "g2": 10.8,
                        "ace1": 3.5, "ace2": 2.8,
                        "df1": 2.1, "df2": 3.0,
                        "o_win1": float(ev.get("odds", {}).get("home", 1.70)),
                        "o_win2": float(ev.get("odds", {}).get("away", 2.10)),
                        "o_u215": float(ev.get("odds", {}).get("under_21_5", 1.80)),
                        "o_set20": float(ev.get("odds", {}).get("home_2_0", 2.50))
                    }
        except Exception:
            pass

    # 2. Recupero da The-Odds-API (Quote Live)
